...
# 插入语句
sql = "insert into book(id,`name`, title, text, url, sort) values(null,%s,%s,%s,%s,%s)"
...
def main(cur, conn):
    try:
        ...
        print("成功获取书籍！")
        name = soup.find("div", class_="cover").find("img")['alt']
        html.close()
...
def getText(name, cur, conn, j):
    try:
        ...
        t_html = urllib.request.urlopen(t_req)
        t_soup = BeautifulSoup(t_html, "lxml")
        title = t_soup.find("h1", class_="wap_none").text
        text = t_soup.find("div", class_="Readarea ReadAjax_content")
        print("获取文章成功："+title)
        cur.execute(sql, (name, title, text, t_url, j))
        conn.commit()
        t_html.close()
